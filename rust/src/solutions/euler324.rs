...
use num::{cast::ToPrimitive, pow::Pow, BigInt};

// Using recursive formula from http://oeis.org/A028452

pub fn euler324() -> String {
    let p = 100000007i128;
    let lrr = LRR::<i128>::new(
        [
            "229",
            "117805",
            "64647289",
            ...
            "446743654489197568088617503727278115945835626935048667406598225",
        ]
        .iter()
        .map(|x| {
            (x.parse::<BigInt>().unwrap() % BigInt::from(p))
                .to_i128()
                .unwrap()
        })
        .collect(),
    ...
    let n = BigInt::from(10).pow(10000u32) / BigInt::from(2) - BigInt::from(1);
    lrr.at_mod(n, p).to_string()
}
